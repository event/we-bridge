 ...
 *               2011 Leonid Movshovich (event.riga@gmail.com)
...
(function($){
	...
	return this.each(function() {
		...
		var results = $('<div />');
		var currentSelection, pageX, pageY;
		...
		function buildResults(resultObjects, sFilterTxt) {
		    ...
		    var filterPatt = settings.caseSensitive ? 
			new RegExp(sFilterTxt, "g") : new RegExp(sFilterTxt, "ig");
		...
		function runSuggest(e) {	
			...
			resultObjects 
			    = searchData(this.value, settings.wildCard
					 , settings.caseSensitive
					 , settings.notCharacter
					 , function(res) {buildResults(res, sFilterTxt);});				
		    } else {
		...
		$(results).addClass('jsonSuggestResults').
		    css({'bottom': ($(window).height() - obj.offset().top + 5) + 'px'
		...
		obj.after(results).
		    ...
		    focus(function(e) {
			    $(results).css({'bottom': ($(window).height() - obj.offset().top + 5) + 'px'
					, 'left': (obj.offset().left + 10) + 'px'
					});
